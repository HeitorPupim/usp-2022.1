...
public abstract class TraversalStrategy {
    ...
    protected void printVisitTree() {
        ...
        treeStringBuilder.append(getGraph().getVertices().get(0).getPosicaoX()).append('\n');
        for (var i = 1; i < getGraph().getNumberOfVertices(); i++) {
            ...
            treeStringBuilder.append(getGraph().getVertices().get(i).getPosicaoX())
                    .append(" \t").append(getDistanceToVertex(i)).append('\n');
        }
        var treeString = treeStringBuilder.toString();
        LOGGER.info(treeString);
    }
    public abstract float[][] getDistanceMatrix();

    public void verticeMaisDistante(float[][] distanceMatrix,int id) {
        float maxDistancia = distanceMatrix[id][0];
        int idMaisDistante=0;
        for (int i = 0; i < distanceMatrix[id].length; i++) {
            if(maxDistancia<distanceMatrix[id][i] && id != i) {
                maxDistancia = distanceMatrix[id][i];
                idMaisDistante = i;
            }
        }
        var xResposta=getGraph().getVertices().get(idMaisDistante).getPosicaoX() ;
        var yResposta = getGraph().getVertices().get(idMaisDistante).getPosicaoY();
        System.out.printf( xResposta +","+ yResposta);

    }

//////////
    public void verticeCentra(float[][] distanceMatrix) {
        var maxDistancia = new float[distanceMatrix.length];
        Arrays.fill(maxDistancia,0);
        for (int i = 0; i < distanceMatrix.length; i++) {
            for (int j = 0; j < distanceMatrix[0].length; j++) {
                    if (maxDistancia[i] < distanceMatrix[i][j] && i != j ) {
                        maxDistancia[i] = distanceMatrix[i][j];
                    }else {
                        //System.out.printf("i: "+i+"  j: "+j+"\n");
                    }
            }
            /*System.out.println(maxDistancia[i]+"  ");
            System.out.println(" ");*/
        }
        int menorIndice = 0;
        ...
        var xResposta=getGraph().getVertices().get(menorIndice).getPosicaoX() ;
        var yResposta = getGraph().getVertices().get(menorIndice).getPosicaoY();
        System.out.printf( xResposta +","+ yResposta+"\n");
    }
    //////
    public void verticePeriferico(float[][] distanceMatrix) {
        var minDistancia = new float[distanceMatrix.length];
        for (int i = 0; i < distanceMatrix.length; i++) {
            for (int j = 0; j < distanceMatrix[0].length; j++) {
                if (minDistancia[j] < distanceMatrix[j][i] && distanceMatrix[j][i] != 0 && i!=j) {
                    minDistancia[j] = distanceMatrix[j][i];
                }
            }
        }
        int menorIndice = 0;
        float maiorDis = minDistancia[0];
        for (int i = 0; i < minDistancia.length; i++) {
            if (maiorDis < minDistancia[i]) {
                maiorDis = minDistancia[i];
                menorIndice = i;
            }
        }
        ...
        var xResposta=getGraph().getVertices().get(menorIndice).getPosicaoX() ;
        var yResposta = getGraph().getVertices().get(menorIndice).getPosicaoY();
        var idResposta = getGraph().getVertices().get(menorIndice).getId();
        System.out.printf( xResposta +","+ yResposta+","+"\n");

        verticeMaisDistante(distanceMatrix,idResposta);
    }
}